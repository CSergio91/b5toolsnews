...
import { DraftDebugModal } from '../components/Builder/DraftDebugModal';
...
const BuilderWizard = () => {
    ...
    const handleSave = async () => {
        ...
        // New Check: Tiebreakers Validation
        // If state.config.tiebreaker_rules is empty or all rules are inactive
        const hasActiveTiebreaker = state.config.tiebreaker_rules && state.config.tiebreaker_rules.some(r => r.active);

        // If rules are not even initialized, they are effectively inactive (since we default to active:false now, but we check truthiness)
        // Actually, if it's undefined, it's NOT initialized, but our new logic in ConfigStep inits them as false.
        // However, if the user hasn't visited ConfigStep, they might be undefined.
        // If undefined, we assume NO rules selected.

        if (!hasActiveTiebreaker) {
            const goToConfig = confirm(
                "⚠️ No has definido ninguna regla de desempate.\n\n" +
                "Es recomendable configurar al menos una regla para evitar empates en la clasificación.\n\n" +
                "¿Quieres ir a Configuración ahora para definirlas?"
            );
            if (goToConfig) {
                setStep(0); // Go to Config
                return;
            }
        }
    ...
    const handlePreview = () => {
        ...
        // Also ensure sub-keys are updated immediately for the debugger
        ...
        if (id) {
            ...
            addToast('Guarda el torneo primero para ver la vista previa', 'warning');
        }
    };
            ...
            <div className="h-screen w-full bg-[#0a0a0a] text-white flex flex-col overflow-hidden">
                ...
                <header className="h-16 border-b border-white/10 flex items-center justify-between px-4 lg:px-6 bg-[#111] z-20 relative">
                    ...
                    <div className="flex items-center gap-2 lg:gap-3">
                        ...
                        <button
                            title="Inspector de Borrador"
                            onClick={handlePreview}
                            className="p-2 md:px-4 md:py-2 bg-yellow-600 hover:bg-yellow-500 text-white rounded-full transition-colors flex items-center gap-2"
                        >
                            <MonitorPlay size={18} />
